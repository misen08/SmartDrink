...
public class BotellaExt {
	...
	public ArrayList<BotellaExt> parsearSabor (String response) {
		JSONObject responseReader;
		ArrayList<BotellaExt> listSaboresEnBotella = new ArrayList<BotellaExt>();
		...
		try {
			...
			if("0".equals(codigoError.toString())){
				...
				for (int i = 0; i < saboresEnBotella.length(); i++) {
					...
					String descripcion = sabor.getString("distancia");
					String habilitado = sabor.getString("disponible");

					BotellaExt botellaExt = new BotellaExt(idBotella, idSabor, descripcion, habilitado);
					listSaboresEnBotella.add(botellaExt);
				}
			} else {
